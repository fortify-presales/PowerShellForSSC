...
$Author = 'Kevin Lee'
$PowerShellForSSC = 'PowerShellForSSC'
...
elseif ($env:APPVEYOR_PULL_REQUEST_NUMBER -gt 0) {
    ...
    Try {
        ...
        $manifestPath = ".\$PowerShellForSSC\$PowerShellForSSC.psd1"
        ...
        $functionList = ((Get-ChildItem -Path .\$PowerShellForSSC\Public).BaseName)
        $splat = @{
            'Path'              = $manifestPath
        ...
        (Get-Content -Path $manifestPath) -replace "PSGet_$PowerShellForSSC", "$PowerShellForSSC" | Set-Content -Path $manifestPath
        (Get-Content -Path $manifestPath) -replace 'NewManifest', $PowerShellForSSC | Set-Content -Path $manifestPath
    ...
    if ($SkipPublishing) {
        ...
        Try
        {
            ...
            $PM = @{
                Path = ".\$PowerShellForSSC"
                NuGetApiKey = $env:NuGetApiKey
                ErrorAction = 'Stop'
            }

            Publish-Module @PM
            Write-Host "$PowerShellForSSC PowerShell Module version $newVersion published to the PowerShell Gallery." -ForegroundColor Cyan
        }
        Catch
        {
            # Sad panda; it broke
